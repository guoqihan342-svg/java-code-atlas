...
def test_jar_path_construction_from_config(sample_config: dict):
    ...
    assert analyzer.jar_path == Path("java-analyzer/target/jstruct-analyzer-0.2.0.jar")
    command = analyzer._build_command("analyze")
    assert "io.github.jstruct.AnalyzerCli" in command
    assert "--root" in command
    assert sample_config["sources"]["root"] in command


def test_analyze_subprocess_call_constructed_correctly(monkeypatch: pytest.MonkeyPatch, sample_config: dict, sample_jstruct_data: dict, tmp_path: Path):
    calls: list[list[str]] = []
    ...
    def fake_run(cmd, capture_output, text, timeout, check):
        ...
        output_path.write_text(json.dumps(sample_jstruct_data), encoding="utf-8")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")

    monkeypatch.setattr("src.orchestrator.subprocess.run", fake_run)
    output_path = tmp_path / "jstruct-raw.json"

    result = JavaAnalyzer(sample_config).analyze(output_path)

    assert result["jstruct"]["version"] == JavaAnalyzer.CURRENT_SCHEMA_VERSION
    assert calls
    cmd = calls[0]
...
def test_error_handling_when_analyzer_process_fails(monkeypatch: pytest.MonkeyPatch, sample_config: dict, tmp_path: Path):
    ...
    with pytest.raises(AnalyzerError, match="bad config"):
        JavaAnalyzer(sample_config).analyze(tmp_path / "jstruct-raw.json")
