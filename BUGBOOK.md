...
**根因**：`JStructDocument` 没有版本字段，两端各自假设 schema 一致。

**修复**：`jstruct.version = "1.0.0"`，Python 端加载时校验版本号。不匹配→报错+提示重建 JAR。
...
**修复**：HTML 只含渲染逻辑 (<100KB)，数据通过 `fetch('/api/jstruct.json')` 外部加载。渐进渲染。
